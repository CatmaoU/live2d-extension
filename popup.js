...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function detectBrowserMemory(callback) {
   ...
   themeToggle.addEventListener('click', async () => {
     ...
     // 手动切换主题时强制关闭跟随系统主题（popup2 中的开关）
     await storage.set({ followSystemTheme: false });
     const s2 = JSON.parse(localStorage.getItem('live2dExtensionSettings') || '{}');
     s2.followSystemTheme = false;
     localStorage.setItem('live2dExtensionSettings', JSON.stringify(s2));
     // 同步到 localStorage
     const settings = JSON.parse(localStorage.getItem('live2dExtensionSettings') || '{}');
     settings.theme = isDark ? 'dark' : 'light';
     localStorage.setItem('live2dExtensionSettings', JSON.stringify(settings));
